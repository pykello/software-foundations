...
Proof.  
  ...
  Case "n = 0".
    simpl. reflexivity.
  Case "n = S n'".
    simpl. rewrite -> IHn'. rewrite (plus_comm n' (S n')). simpl. reflexivity. 
  Qed. 
(** [] *)
...
Theorem mult_dist : forall n p q : nat,
 n * (p + q) = n * p + n * q.
Proof.
 intros n p q.
 induction n as [|n'].
  ...
  Case "n = S n'".
   simpl.
   rewrite -> IHn'.
   rewrite -> (plus_assoc (p + q) (n' * p) (n' * q)).
   rewrite -> (plus_comm p q).
   rewrite <- (plus_assoc q p (n' * p)).
   rewrite -> (plus_comm q (p + n' * p)).
   rewrite <- (plus_assoc (p + n' * p) q (n' * q)).
   reflexivity.
 Qed.


Lemma mult_1 : forall n : nat,
 n * 1 = n.
Proof.
 intros n.
 induction n as [|n'].
...
Proof.
  ...
  intros H1.
  induction p as [|p'].
   Case "p = 0". simpl. rewrite H1. reflexivity.
...
Proof.
  ...
  induction m as [|m'].
    ...
    Case "m = S m'".
     simpl.
     rewrite (plus_1 m').
     rewrite (mult_dist n p (m' * p)).
     rewrite (mult_dist n m' 1).
     rewrite (mult_comm (n * m' + n * 1) p).
     rewrite (mult_dist p (n * m') (n * 1)).
     rewrite plus_comm.
     rewrite mult_1.
...
Theorem bin_conv_inc_comm: forall n: bin,
  bin2unary(bininc(n)) = S (bin2unary(n)).
Proof.
 intros n.
 induction n as [ | | n' | n'].
  Case "n = Z".
  ...
  Case "n = twiceAdd1 n'".
   simpl.
   rewrite (plus_0_r (bin2unary (bininc n'))).
   rewrite (plus_0_r (bin2unary n')).
   rewrite IHn'.
   simpl.
   rewrite (plus_comm (bin2unary n') (S (bin2unary n'))).
   simpl.
   reflexivity.
Qed.
...
Fixpoint unary2bin (n: nat) : bin :=
  match n with
  | 0 => Z
  | 1 => I
  | S n' => bininc (unary2bin (n'))
  end.

Theorem binary_next: forall n: nat,
  unary2bin(S n) = bininc(unary2bin(n)).
Proof.
  destruct n as [|n'].
   Case "0". reflexivity.
...
Proof.
  ...
  induction n as [|n'].
    ...
    Case "n = S n'".
     rewrite binary_next.
     rewrite bin_conv_inc_comm.
     rewrite IHn'.
     reflexivity.
Qed.

Definition normalize (n: bin) : bin :=
  unary2bin(bin2unary(n)).

Example normalize_1:
  normalize (twice (twice (Z))) = Z.
Proof. reflexivity. Qed.

Example normalize_2:
  normalize (twice (twiceAdd1 (I))) = twice (twiceAdd1 (I)).
Proof. reflexivity. Qed.

Theorem G1: forall n: bin,
  unary2bin(bin2unary(n)) = normalize(n).
Proof.
  intros n.
  reflexivity.
Qed.

Theorem unary_inverse: forall n: bin,
  normalize (unary2bin(bin2unary(n))) = normalize (n).
Proof.
  intros n.
  rewrite <- G1.
  rewrite <- G1.
  rewrite binary_inverse.
  reflexivity.
Qed.
...
(* $Date: 2014-11-10 13:37:33 -0500 (Mon, 10 Nov 2014) $ *)
